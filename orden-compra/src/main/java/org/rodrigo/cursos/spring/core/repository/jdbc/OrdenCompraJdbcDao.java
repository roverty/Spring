...
public class OrdenCompraJdbcDao extends GenericJdbcDao implements OrdenCompraRepository {
	...
	private static final String ordenCompraQueryPrefix = "select orden_compra_id, fecha_compra, "
	...
	public void guarda(OrdenCompra orden) {
		...
		orden.setId(id);
		rows = getJdbcTemplate().update(creaOrdenQuery,orden.getId(),
				orden.getFechaCompra(),orden.getStatus().getId(),orden.getFechaStatus());
	...
	public boolean existeOrden(OrdenCompra orden) {
		...
		count = getJdbcTemplate().queryForObject(existeOrdenCompraQuery, 
				Integer.class,orden.getId());
	...
	public List<OrdenCompra> buscaOrdenes(Date fechaCompraInicio, Date fechaCompraFin, 
		...
		return getJdbcTemplate().query(query,OrdenCompraJdbcDao::ordenCompraRowMapper,array);
	}
	
	private static OrdenCompra ordenCompraRowMapper(ResultSet rs, int rowNum) throws SQLException {
		OrdenCompra orden;
		int numCol = 1;
